...
const Ruler = (props: RulerProps) => {
  ...
  const beatsT = useVmontage((s) => s.state?.beats?.beats_t);
  ...
  const draw = useCallback(
    (context: CanvasRenderingContext2D, scrollLeft: number, width: number, height: number) => {
      ...
      // vmontage: 拍点刻度(音乐节拍,切点应吸附于此)
      if (beatsT?.length) {
        context.fillStyle = "#f59e0b";
        context.globalAlpha = 0.9;
        for (const beat of beatsT) {
          const x = Math.floor(beat * pixelsPerSecond - scrollLeft + offsetX) + 0.5;
          if (x < -4) continue;
          if (x > width) break;
          context.beginPath(); // 底边小三角
          context.moveTo(x - 3, height);
          context.lineTo(x + 3, height);
    ...
    [scale.zoom, pixelsPerSecond, offsetX, durationUs, colors.text, colors.border, beatsT],
  );

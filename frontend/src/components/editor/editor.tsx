...
import { ChatPanel } from "./vmontage/chat-panel";
...
export default function Editor({
  ...
  // 启动即加载 vmontage 后端的 edl.json;后端不可用时回退演示模板
  const vmInit = useVmontage((s) => s.init);
  const vmLoad = useVmontage((s) => s.loadIntoEditor);
  useEffect(() => {
    resetProject();
    core.project.new();
    const timer = setTimeout(async () => {
      try {
        await vmInit();
        const st = useVmontage.getState().state;
        if (st?.edl?.timeline?.length) {
          vmLoad();
          return;
        }
      } catch {}
      core.project.import(data); // 后端没起/没有 EDL:回退官方示例
    }, 500);
    return () => clearTimeout(timer);
  }, [resetProject, vmInit, vmLoad]);

...
- 主业:逐个审视片段内击杀间隙,三选一 keep/compress/cut;击杀一个不能丢;
...
2. 逐个审视片段内每个击杀间隙(相邻击杀时间戳之差):
   - keep:间隙 < 3s,或期间可能有值得看的内容(残局对峙、下包、关键走位)
   - compress:长距离转场/找人 → 该跨度变速 4~8 倍(from_t 距上一杀 >= 1.0s,
     to_t 距下一杀 >= 1.5s,给观众常速缓冲)
   - cut:间隙毫无价值 → 直接剪掉该跨度(同样留出上述缓冲)
...
{{"shots": [{{"clip_id": "...", "in_t": 秒, "out_t": 秒,
  "gap_treatments": [{{"from_t": 秒, "to_t": 秒, "action": "keep|compress|cut",
                      "factor": 倍速或null, "rationale": "..."}}],
  "rationale": "..."}}],
...
def validate_storyboard(sb: Storyboard, cards: ScorecardsFile,
    ...
    factor_lo, factor_hi = settings["director"]["factor_range"]
    ...
    for shot in sb.shots:
        ...
        for g in sorted(shot.gap_treatments, key=lambda g: g.from_t):
            ...
            if g.action == "compress":
                if g.factor is None or not (factor_lo <= g.factor <= factor_hi):
                    warnings.append(f"{shot.clip_id}: 压缩倍速 {g.factor} 超出 "
                                    f"[{factor_lo},{factor_hi}],已剔除该处理。")
                    continue
